...
class ProviderLane:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not self.credential_env_names
            or len(self.credential_env_names) > 4
            or any(not name.strip() for name in self.credential_env_names)
        ):
        ...
        if not 1 <= self.max_rounds <= 8:
            raise ValueError("provider lane max_rounds must be between 1 and 8")
