...
OPTIONAL_POST_PROCESSING_NEED_TYPES = frozenset({"readme", "readme_generation"})
...
class ToolPlanningTaskExecutor:
    ...
    def execute_task(self, task: Task, context: TaskExecutionContext) -> TaskExecutionResult:
        ...
        try:
            ...
            preselected_tool_requests = self._preselected_tool_requests(task)
            initial_tool_requests = (
                preselected_tool_requests
                if preselected_tool_requests is not None
                else self._typed_validation_tool_requests(task)
            )
            if preselected_tool_requests is not None:
                self._log(
                    "preselected_tool_planning_started",
                    input_summary={"task_id": task.id, "goal": goal},
                    output_summary={"tool_count": len(initial_tool_requests)},
                    success=None,
                )
            elif initial_tool_requests is None:
    ...
    def _preselected_tool_requests(self, task: Task) -> list[dict[str, Any]] | None:
        ...
        project_read_count = sum(
            1 for need in validated_needs if need.attributes.get("source_class") == "project"
        )
        if len(validated_needs) > remaining_tool_calls or project_read_count > remaining_file_reads:
            raise DecisionNeedResolutionError(
...
- Emit readme_generation only when its exact target is in the typed write scope.
...
"""
    ...
    def _typed_write_scope_prompt(self) -> str:
        ...
        return (
            ...
            "- Do not plan mutations outside these targets. README or documentation "
    ...
    def _filter_needs_for_task_contract(
        ...
        for raw_need in raw_needs:
            ...
            if need_type in OPTIONAL_POST_PROCESSING_NEED_TYPES:
                attributes = raw_need.get("attributes") if isinstance(raw_need.get("attributes"), dict) else {}
                target = str(raw_need.get("target_path") or attributes.get("file_path") or "").strip()
