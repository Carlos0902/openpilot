...
from core.deepseek_roundtrip import DeepSeekToolRoundTripError, append_deepseek_tool_round_trip
...
def test_deepseek_round_trip_requires_reasoning_when_enabled() -> None:
    results = [LLMToolResult(tool_call_id="call-openai-1", content="source evidence")]

    with pytest.raises(DeepSeekToolRoundTripError, match="reasoning_content"):
        append_deepseek_tool_round_trip([], _response(), results)

    continued = append_deepseek_tool_round_trip(
        [],
        _response(reasoning="hidden state"),
        results,
    )
    assert continued[0].reasoning_content == "hidden state"
