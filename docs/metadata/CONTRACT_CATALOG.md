...
- `RuntimeBudgetMetadata` remains the single owner of both completion-accounting
  ...
  success authority.
- `ProviderBudgetDiagnostic` records one bounded provider attempt, including
...
`ContextCompactionAttempt` distinguishes provider acceptance from builder
selection and records only bounded provenance. `ContextCompactionReuseAdmission`
is shadow-only: admitted evidence still cannot enter a prompt. Reusable bindings
may add a source hash, while historical bindings without that field remain
readable.
