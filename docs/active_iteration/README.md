...
当前实施状态：持久化对话轮次元数据、durable store 与 assistant ledger 幂等提交/崩溃恢复已完成；
下一切片是 canonical prepared/active task binding 及 session authority freshness/revocation
resume gate。统一 CLI 入口尚未启用。
